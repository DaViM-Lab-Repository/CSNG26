...
const OptimizedAdjacencyMatrixRenderer = () => {
  ...
  // Get filtered adjacency data and create compact matrix mapping
  const { filteredAdjacencyData, dimensions, indexMapping, reverseIndexMapping } = React.useMemo(() => {
    if (!selectedNodes || selectedNodes.length === 0) {
      return {
        filteredAdjacencyData: dGraphData,
        dimensions: { width: segments.length, height: segments.length },
        indexMapping: null,
        reverseIndexMapping: null
      };
    }
    
    // Get all member indices from selected communities
    const communityMembers = new Set();
    selectedNodes.forEach(node => {
      if (node.members) {
        node.members.forEach(memberIdx => {
          communityMembers.add(parseInt(memberIdx));
        });
      }
    });
    ...
    // Create sorted array of community members for consistent ordering
    const sortedMembers = Array.from(communityMembers).sort((a, b) => a - b);
    
    // Create mapping from original indices to compact matrix indices
    const indexMapping = new Map();
    const reverseIndexMapping = new Map();
    sortedMembers.forEach((originalIndex, compactIndex) => {
      indexMapping.set(originalIndex, compactIndex);
      reverseIndexMapping.set(compactIndex, originalIndex);
    });
    
    // Create compact adjacency matrix
    const compactMatrix = [];
    sortedMembers.forEach((i, compactI) => {
      if (dGraphData[i]) {
        const compactConnections = [];
        dGraphData[i].forEach(j => {
          if (communityMembers.has(j)) {
            compactConnections.push(indexMapping.get(j));
          }
        });
        if (compactConnections.length > 0) {
          compactMatrix[compactI] = compactConnections;
        }
      }
    });
    
    return {
      filteredAdjacencyData: compactMatrix,
      dimensions: { width: sortedMembers.length, height: sortedMembers.length },
      indexMapping,
      reverseIndexMapping
    };
  }, [dGraphData, selectedNodes, segments.length]);
  ...
  const renderBatch = useCallback((ctx, visibleArea, startBatchY, isFirstBatch = false) => {
    ...
    // Helper function to find streamline index for a given matrix position
    const findStreamlineIndex = (pos) => {
      // If we have a compact matrix, convert compact index back to original index
      if (reverseIndexMapping) {
        const originalPos = reverseIndexMapping.get(pos);
        if (originalPos === undefined) return 0;
        pos = originalPos;
      }
      
      if (!streamLines || streamLines.length === 0) return 0;
      for (let idx = 0; idx < streamLines.length; idx++) {
        if (pos >= streamLines[idx][0] && pos < streamLines[idx][1]) {
          return idx;
        }
      }
      return streamLines.length - 1; // Fallback to last streamline if not found
    };
    ...
    for (let i = startBatchY; i < endBatchY; i++) {
      for (let j = visibleArea.startX; j < visibleArea.endX; j++) {
        const streamLineI = findStreamlineIndex(i);
        const streamLineJ = findStreamlineIndex(j);
    ...
    for (let i = startBatchY; i < endBatchY; i++) {
      if (filteredAdjacencyData[i]) {
        filteredAdjacencyData[i].forEach(j => {
  ...
  }, [viewState, streamLines, filteredAdjacencyData]);
  ...
  // Helper function to find streamline index for a given matrix position
  const findStreamlineIndex = useCallback((pos) => {
    if (!streamLines || streamLines.length === 0) return 0;
    for (let idx = 0; idx < streamLines.length; idx++) {
      if (pos >= streamLines[idx][0] && pos < streamLines[idx][1]) {
        return idx;
  ...
  const handleMouseDown = useCallback((e) => {
    ...
    if (e.button === 2) {
      ...
      // Convert compact matrix coordinates back to original indices if needed
      let originalMatrixX = matrixX;
      let originalMatrixY = matrixY;
      
      if (reverseIndexMapping) {
        originalMatrixX = reverseIndexMapping.get(matrixX) ?? matrixX;
        originalMatrixY = reverseIndexMapping.get(matrixY) ?? matrixY;
      }
      
      // Get streamline indices using findStreamlineIndex with original indices
      const streamLineI = findStreamlineIndex(originalMatrixY);
      const streamLineJ = findStreamlineIndex(originalMatrixX);
  ...
  }, [viewState.scale, streamLines, segments, setSelectedSegments, getCellSize, reverseIndexMapping, findStreamlineIndex]);
  ...
  // Add event listener for the "setPixels" event from the Render button
  useEffect(() => {
    const handleSetPixelsEvent = () => {
      // Force a re-render when the Render button is clicked
      render();
    };
  ...
  // Re-render when selected nodes change (community selection changes)
  useEffect(() => {
    render();
  }, [selectedNodes, render]);
