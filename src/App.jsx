...
const App = () => {
  ...
  // Initialize global runtime tracking functions
  useEffect(() => {
    // Initialize window.runtimes if it doesn't exist
    if (!window.runtimes) {
      window.runtimes = [];
    }

    // Function to calculate average runtime for each algorithm name
    window.average = () => {
      if (!window.runtimes || window.runtimes.length === 0) {
        return {};
      }

      const grouped = {};
      window.runtimes.forEach(entry => {
        if (!grouped[entry.name]) {
          grouped[entry.name] = [];
        }
        grouped[entry.name].push(entry.runtime);
      });

      const averages = {};
      Object.keys(grouped).forEach(name => {
        const runtimes = grouped[name];
        const sum = runtimes.reduce((acc, runtime) => acc + runtime, 0);
        averages[name] = sum / runtimes.length;
      });

      return averages;
    };

    // Function to calculate total average of all algorithm averages
    window.totalAverage = () => {
      const averages = window.average();
      const algorithmAverages = Object.values(averages);
      
      if (algorithmAverages.length === 0) {
        return 0;
      }

      const sum = algorithmAverages.reduce((acc, avg) => acc + avg, 0);
      return sum / algorithmAverages.length;
    };
  }, []);
  ...
  }, [segments, selectedRenderingWindows]);
